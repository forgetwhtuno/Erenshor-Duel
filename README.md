...
The deterministic self-test suite is wired for the current source snapshot, but it could not be executed in this sandbox because no PowerShell/C# compiler is installed. A fresh installed-reference build and plugin-identity audit are also pending because the supplied packet does not include the current game `Assembly-CSharp.dll`. Version 0.4.4 preserves the 0.4.3 nested damage stack, world-real ledger, self-cast repair, actor/effect-aware AoE containment, status/effect restoration, lifecycle, and Guard/follow cleanup while restoring the live-proven 0.4.1 participant-damage transaction semantics. Full live verification remains required for repeated melee, direct spells/DoTs/retaliation, self-heal with the opponent selected, AoE with hostile/protected bystanders, legitimate yield, immediate repeat, post-duel vanilla combat, zoning, and unload/reload.
