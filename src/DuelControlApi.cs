...
namespace ErenshorDuel
{
    ...
    public static class DuelControlApi
    {
        ...
        {
            return new DuelControlState { Active = DuelController.Active, Status = DuelController.Status(), EligibleNames = DuelController.EligibleNames() };
        }
        public static string GetStatus()
        {
            string[] names = DuelController.EligibleNames();
            int count = names == null ? 0 : names.Length;
            return DuelController.Active ? "Duel active" : "Idle | " + count + " eligible local candidate(s)";
        }
